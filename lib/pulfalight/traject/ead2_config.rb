...
each_record do |_record, context|
  ...
  context.clipboard[:repository] = repository.name if repository

  context.clipboard[:record] = record
end
...
def index_child_components
  # rubocop:disable Metrics/BlockLength

  compose "components", ->(record, accumulator, _context) { accumulator.concat record.xpath("//dsc[@type='combined']//*[is_component(.)]", NokogiriXpathExtensions.new) } do |*_args|
  ...
  # self.index_child_components
end

index_child_components
